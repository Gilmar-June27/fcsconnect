...
if (isset($_POST['update_order'])) {
    ...
    $update_payment = $_POST['update_order']; // "processing" or "completed"
    ...
    $message = "Your Order status has been updated to $update_payment";
    $msg = "$admin_name";
    ...
    // Update the top sales information (optional)
    updateTopSales($conn);

    // Directly output the success message
    echo '<p class="alert alert-success">Order has been ' . ucfirst($update_payment) . '!</p>';

    header('location:admin_processing_order.php');
}
...
// if (isset($_POST['update_order'])) {
//     $order_update_id = $_POST['order_id'];
//     $update_payment = $_POST['update_payment']; // Now defaults to "completed" when the button is clicked.
...
$message_count = mysqli_fetch_assoc($select_messages)['count'];
  ...
  <div class="container mt-4">
    <h1 class="text-center text-success mt-4">Pending Orders</h1>
    ...
    <div class="table-responsive">
      <table class="table table-striped table-bordered">
            ...
            <tbody>
                <?php
                $select_orders = mysqli_query($conn, "SELECT * FROM `orders` WHERE admin_id='$admin_id' AND payment_status='pending'") or die('query failed');
                ...
                <tr>
                    ...
                    <td><a href="admin-chatsystem.php?user_id=<?php echo htmlspecialchars($fetch_orders['user_id']); ?>"><?php echo $fetch_orders['name']; ?></a></td>
    ...
    <!-- Hidden field to update payment status -->
    <input type="hidden" name="update_payment" value="completed">

    <!-- Button for Processing status -->
    <button type="submit" name="update_order" value="processing" class="btn btn-warning" <?php if($fetch_orders['payment_status'] == 'processing') echo 'disabled'; ?>>Processing</button>
    
    <!-- Button for Completed status -->
    <!-- <button type="submit" name="update_order" value="completed" class="btn btn-primary" <?php if($fetch_orders['payment_status'] == 'processing') echo 'disabled'; ?>>Completed</button> -->
</form>
