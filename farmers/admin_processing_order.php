...
if (isset($_POST['update_payment'])) {
    ...
    $message = $update_payment === 'out_of_delivered' ? "Your Order is out for delivery" : "Your Order has been completed";
    $msg = "$admin_name";
    
    $escaped_message = mysqli_real_escape_string($conn, $message);
    $escaped_msg = mysqli_real_escape_string($conn, $msg);
    
    // Insert notification with updated message
    ...
    updateTopSales($conn);
    echo '<p class="alert alert-success">Order status has been updated!</p>';
    
    // Redirect to a specific page if necessary
    header("location:out_of_deliver_order.php");
    exit();
}
...
$message_count = mysqli_fetch_assoc($select_messages)['count'];
  ...
  <div class="container mt-4">
    <h1 class="text-center text-success mt-4">Processing Orders</h1>
    ...
    <div class="table-responsive">
      <table class="table table-striped table-bordered">
            ...
            <tbody>
                <?php
                $select_orders = mysqli_query($conn, "SELECT * FROM `orders` WHERE admin_id='$admin_id' AND payment_status='processing'") or die('query failed');
                if(mysqli_num_rows($select_orders) > 0){
                    while($fetch_orders = mysqli_fetch_assoc($select_orders)){
                        $is_completed = isset($fetch_orders['payment_status']) && $fetch_orders['payment_status'] === 'out_of_delivered';
                ?>
                <tr>
    ...
    <input type="hidden" name="update_payment" value="out_of_delivered">

    <!-- Button for Processing status -->
    <button type="submit" name="update_order" value="processing" class="btn btn-warning" <?php if($fetch_orders['payment_status'] == 'completed') echo 'disabled'; ?>>Completed</button>
    
    <!-- Button for Completed status -->
    <button type="submit" name="update_order" value="out_of_delivered" class="btn btn-primary" <?php if($fetch_orders['payment_status'] == 'out_of_delivered') echo 'disabled'; ?>>Out Of Delivery</button>
</form>
