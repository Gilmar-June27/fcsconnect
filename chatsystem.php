...
// Check if the user has blocked the admin
$block_check_query = "SELECT * FROM `reports` WHERE reporter_id = '$user_id' AND reported_user_id = '$admin_id' AND blocked = 1";
...
$blocked = mysqli_num_rows($block_check_result) > 0;

// Fetch admin details
$admin_query = mysqli_query($conn, "SELECT * FROM `users` WHERE id = '$admin_id'") or die('Query failed: ' . mysqli_error($conn));
...
// Fetch user details
$user_query = mysqli_query($conn, "SELECT * FROM `users` WHERE id = '$user_id'") or die('Query failed: ' . mysqli_error($conn));
...
if (isset($_POST['send_message']) && (!empty($_POST['message']) || !empty($image_path))) {
    ...
    $insert_query = "INSERT INTO `message` (user_id, name, email, number, message, image, admin_id) VALUES ('$user_id', '{$user_data['name']}', '{$user_data['email']}', '$user_number', '$message_content', '$image_path', '$admin_id')";
    mysqli_query($conn, $insert_query) or die('Query failed: ' . mysqli_error($conn));

    // Redirect to avoid form resubmission on page reload
    header('Location: ' . $_SERVER['PHP_SELF'] . '?admin_id=' . urlencode($admin_id));
    exit;
}
        ...
        <div class="col-md-8 offset-md-2">
            <div class="card shadow-lg">
                <div class="card-header bg-primary text-white d-flex align-items-center">
                    <img src="<?php echo !empty($admin_data['image']) ? 'images/' . htmlspecialchars($admin_data['image']) : $default_image_url; ?>" alt="<?php echo htmlspecialchars($admin_data['name']); ?>'s image" class="rounded-circle mr-3" style="width: 50px; height: 50px;">
                    <h5 class="mb-0"><?php echo htmlspecialchars($admin_data['name']); ?></h5>
                </div>
                <div class="card-body message-display p-4" style="height: 400px; overflow-y: auto;">
                    <?php if ($blocked): ?>
                        <div class="alert alert-danger">You have blocked this farmer and cannot send messages.</div>
                    <?php else: ?>
                        <?php
                        if (mysqli_num_rows($messages_query) > 0) {
                            while ($message = mysqli_fetch_assoc($messages_query)) {
                                ...
                                ?>
                                <div class="d-flex align-items-start mb-4 <?php echo $is_user_message ? 'justify-content-end' : ''; ?>">
                                    <?php if (!$is_user_message): ?>
                                        <img src="<?php echo !empty($admin_data['image']) ? 'images/' . htmlspecialchars($admin_data['image']) : $default_image_url; ?>" alt="<?php echo htmlspecialchars($admin_data['name']); ?>'s image" class="rounded-circle m-2" style="width: 40px; height: 40px;">
                                    <?php endif; ?>
                                    ...
                                    <?php if ($is_user_message): ?>
                                        <img src="<?php echo !empty($user_data['image']) ? 'images/' . htmlspecialchars($user_data['image']) : $default_image_url; ?>" alt="<?php echo htmlspecialchars($user_data['name']); ?>'s image" class="rounded-circle ml-2" style="width: 40px; height: 40px;">
                                    <?php endif; ?>
                                </div>
                                <?php
                            }
                ...
                <div class="card-footer bg-light">
                    <form id="messageForm" action="" method="POST" enctype="multipart/form-data" class="d-flex">
                        <textarea name="message" id="messageInput" class="form-control mr-2" rows="2" placeholder="Type your message here..."></textarea>
                        ...
                        <button type="submit" id="sendMessageBtn" name="send_message" class="btn btn-primary ml-2">Send</button>
                    </form>
                </div>
            </div>
        </div>
...
$(document).ready(function() {
    ...
    $('#sendMessageBtn').on('click', function(e) {
        ...
        $.ajax({
            url: 'msg-script.php', // The PHP script handling the form data
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function(response) {
                // Clear the message input and image preview after successful submission
                $('#messageInput').val('');
                $('#imagePreview').attr('src', '').hide();
                
                // Optionally, append the new message to the chat without reloading
    ...
    // Update the timestamps in real-time
