...
<section  class="slider section" data-builder="section" data-colorpreset="cp-dark-background" style="    background: rgb(219 213 213 / 10%);">
    ...
    <div class="container section-title" data-aos="fade-up">
      <p><span>Recommended</span> <span class="description-title">Farmers</span></p>
  ...
  <div class="row flex-row flex-nowrap overflow-auto">
        <?php
            $select_products = mysqli_query($conn, "SELECT image, id AS admin_id, name AS admin_name , farmer_select FROM `users` WHERE user_type = 'admin'") or die('query failed');
            if (mysqli_num_rows($select_products) > 0) {
            while ($fetch_products = mysqli_fetch_assoc($select_products)) {
            $admin_image = $fetch_products['image'] ? 'images/' . $fetch_products['image'] : 'images/default-avatar.png';
        ?>
            <div class="card custom-card" style="width: 55vh;margin-right: 15px;">
                  <img src="<?php echo $admin_image; ?>" alt="Admin Image" class="card-img-top" alt="..." style="height: 52vh;">
                  <div class="card-body">
                      <h3><a href="admin-sale_product.php?admin_id=<?php echo $fetch_products['admin_id']; ?>"><?php echo $fetch_products['admin_name']; ?></a></h3>
